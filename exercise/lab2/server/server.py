...
class Server(Bottle):
    ...
    def __init__(self, ID, IP, servers_list, roles_list):
        ...
        #self.route('/', callback=self.setup_election)
    ...
    #def setup_election(self):
     #   self.do_parallel_task_after_delay(10, self.start_election, args=(,))
...
def main():
    ...
    parser.add_argument('--elec_role',
                        nargs='?',
                        dest='roles_list',
                        default="slave",
                        help='This displays the election attribute for each server')
    args = parser.parse_args()
    server_id = args.id
    server_ip = "10.1.0.{}".format(server_id)
    servers_list = args.srv_list.split(",")
    roles_list = args.roles_list
    ...
    try:
        ...
        server.do_parallel_task_after_delay(10, server.start_election, args=()) 
